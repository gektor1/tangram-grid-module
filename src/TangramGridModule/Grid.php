...
/**
 * Grid is the front class. It wants a datasource passed thru the constructor
...
class Grid {
    ...
    /**
     * @var Grid\DataSource\DataSourceInterface
     */
     ...
     * @var string
     */
    protected $_filterToolbar = false;
     ...
     * Constructor
     *
     * @param Grid\DataSource\DataSourceInterface $dataSource
     * @param array                          $options
     */
    public function __construct(Grid\DataSource\DataSourceInterface $dataSource = null) {
        $id = rand(0, 3000);

        $this->setId('pgrid' . $id);

        if ($dataSource instanceof Grid\DataSource\DataSourceInterface) {
    ...
    /**
     * Grid needs to know the data source in order to generate the initial column names etc.
     *
     * @param  Grid\DataSource\DataSourceInterface $dataSource
     * @return self
     */
    public function setDataSource(Grid\DataSource\DataSourceInterface $dataSource) {
     ...
     *
     * @return Grid\DataSource\DataSourceInterface
     */
    ...
    public function setRowsPerPage($amount) {
        ...
        if (null === $this->_dataSource) {
            throw new \Exception('No data source defined');
        } else {
     ...
     * Enable filter
     */
    public function enableFilterToolbar() {
        $this->_filterToolbar = true;
        return $this;
    }
     ...
     * Disables filter
     */
    public function disableFilterToolbar() {
...
EOF;
        ...
        $this->appendAttribute('onCellSelect', new \Zend\Json\Json_Expr("
    ...
    public function getJavascript($pretty = false) {
        ...
        foreach ($this->_dataSource->columns as $name => $column) {
            ...
            if (!isset($column['name'])) {
                throw new \Exception('Column "' . $name . '" is undefined');
            }
    ...
    protected function _getLinkFormatter() {
        ...
        return new \Zend\Json\Json_Expr("function(cellvalue, options, rowObject) {
    ...
    protected function _render(array $settings, $pretty = false) {
        ...
        $json = \Zend\Json\Json::encode($settings, false, array('enableJsonExprFinder' => true));
        if ($pretty) {
    ...
    protected function _mergeAttribute($attribute, $value, $method = 'append') {
        if (!isset($this->_attributes[$attribute])) {
            ...
            if (($currentValue instanceof \Zend\Json\Json_Expr && strpos($currentValue, 'function') !== false)
                    || ($value instanceof \Zend\Json\Json_Expr && strpos($value, 'function') !== false)
            ) {
                ...
                $value = new \Zend\Json\Json_Expr($value);
            } else {
     ...
     *
     * @param  string|\Zend\Json\Json_Expr $function
     * @return string
     */
    ...
    protected function _renderMethods() {
        ...
        foreach ($this->_methods as $method => $options) {
            $jsonOptions = \Zend\Json\Json::prettyPrint(\Zend\Json\Json::encode($options, false, array('enableJsonExprFinder' => true)));
            $output .= sprintf('$("#%s").jqGrid("%s", %s);', $this->_id, $method, $jsonOptions) . PHP_EOL;
        }
